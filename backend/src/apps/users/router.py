...
from src.apps.users.dependencies import get_user_manager
...
async def get_users(user_manager: UserManager = Depends(get_user_manager)) -> list[UserSchema]:
    users = await user_manager.get_users()
    return users


@user_router.get("/{telegram_id}")
async def get_user(telegram_id: int, user_manager: UserManager = Depends(get_user_manager)) -> UserSchema:
...
@user_router.post("/")
async def create_user(user_schema: CreateUserSchema, user_manager: UserManager = Depends(get_user_manager)):
    created_user = await user_manager.create_user(user_schema)
    return created_user


@user_router.put("/{telegram_id}")
async def update_user(telegram_id: int, user_schema: UpdateUserSchema, user_manager: UserManager = Depends(get_user_manager)):
    updated_user = await user_manager.update_user(telegram_id, user_schema)
    return updated_user

@user_router.delete("/{telegram_id}")
async def delete_user(telegram_id: int, user_manager: UserManager = Depends(get_user_manager)):
