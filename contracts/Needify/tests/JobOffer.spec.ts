...
import { JobOffer, storeRedButton } from '../wrappers/JobOffer';
...
describe('JobOffer', () => {
    ...
    beforeEach(async () => {
        ...
        log(platform.address, "platform address")
    ...
    it("Test: Run RedButton flow", async () => {
        await RunPositiveFlow()
        let JOJWData1 = await JOJW.getGetWalletData()
        let JONV1 = await JONW.getGetWalletData()
        let posterJWBefore = await posterJW.getGetWalletData()
        let posterNWBefore = await posterNW.getGetWalletData()
        // Red button
        let red_button_cell = beginCell().store(
            storeRedButton({
                $$type: "RedButton",
                amount_jetton: JOJWData1.balance,
                amount_native: JONV1.balance,
        ...
        let res = await platform.send(
            {
                value: TransferAmount,
                to: jobOffer.address,
                body: red_button_cell
            }
        )
        let JOJWData = await JOJW.getGetWalletData()
        let JONWData = await JONW.getGetWalletData()
        let posterJWDataAfter = await posterJW.getGetWalletData()
        let posterNWDataAfter = await posterNW.getGetWalletData()
        let JobOfferData = await jobOffer.getJobData()
        expect(JobOfferData.balance).toBe(0n)
        expect(JobOfferData.state).toBe(6n) // Closed
        expect(JOJWData.balance).toBe(0n)
        expect(JONWData.balance).toBe(0n)
        expect(posterJWDataAfter.balance).toBe(posterJWBefore.balance + JOJWData1.balance)
        expect(posterNWDataAfter.balance).toBe(posterNWBefore.balance + JONV1.balance)
    });
    ...
    it("Test: should revoke", async () => {
        ...
        let JOJWData1 = await JOJW.getGetWalletData()
        let JONWData1 = await JONW.getGetWalletData()
        log(JOJWData1.balance, "JOJWData1")
        log(JONWData1.balance, "JONWData1")
        let posterJWBefore = await posterJW.getGetWalletData()
        let posterNWBefore = await posterNW.getGetWalletData()
        log(posterJWBefore.balance, "posterJWBefore")
        log(posterNWBefore.balance, "posterNWBefore")
        ...
        log(printTransactionFees(s.transactions))
        log(s.events)
        let JobOfferData = await jobOffer.getJobData()
        log(JobOfferData, "JobOfferData")
        let JOJWData = await JOJW.getGetWalletData()
        log(JOJWData.balance, "JOJWData.balance")
        log(JOJW.address, "address JOJW")
        log(posterJW.address, "address posterJW")

        let posterJWDataAfter = await posterJW.getGetWalletData()
        log(posterJWDataAfter.balance, "posterJWDataAfter")
